...
export ML_DATA=$DATA_DIR

if [[ -z "$NB_SAMPLES" ]] ; then
    CUDA_VISIBLE_DEVICES= ./scripts/create_datasets.py --dataset=mnist
else
    CUDA_VISIBLE_DEVICES= ./scripts/create_datasets.py --dataset=mnist --nb_samples=$NB_SAMPLES
fi
...
#         CUDA_VISIBLE_DEVICES= scripts/create_split.py --seed=$seed --size=$size $ML_DATA/SSL/cifar10 $ML_DATA/cifar10-train.tfrecord &
    CUDA_VISIBLE_DEVICES= scripts/create_split.py --seed=3 --size=$size $ML_DATA/SSL/mnist $ML_DATA/mnist-train.tfrecord
# done
